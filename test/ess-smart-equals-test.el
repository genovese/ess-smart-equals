...
(ert-deftest ess-smart-equals/inferior-narrowing ()
  "Test narrowing in inferior-ess-r-mode."
  (let ((ess-history-file nil)
        (ess-smart-equals-major-mode #'inferior-ess-r-mode)
        lo hi tmp)
    ;; Note: cannot test the process-mark case here
    (with-ess-buffer
      (insert "> \n> ")
      (setq lo (point))
      (insert "if(a")
      (setq hi (point))
      (should (equal (list lo hi) (essmeq--r-repl-current-region))))
    (with-ess-buffer
      (insert "> ")
      (setq lo (point))
      (insert "f(\n+ \n+ )")
    ...
    (with-ess-buffer
      (insert "> g(\n")
      (insert " Error: ill-formed expression\n")
      (insert "> ")
      (setq lo (point))
      (insert "2 ")
      (setq tmp (point))
